...
export const metadata: Metadata = {
  ...
  description: 'Le fotografie dei borghi e delle coste della Riviera dei Fiori: autori e licenze (Wikimedia Commons).',
}
  ...
  return (
    <div className="bg-crema min-h-screen">
      <div className="container mx-auto px-4 md:px-6 py-14 md:py-20 max-w-3xl">
        <p className="font-alt text-xs font-bold uppercase tracking-[0.16em] text-alga mb-2">Il sito</p>
        <h1 className="font-display font-extrabold tracking-tight text-3xl md:text-4xl text-ink mb-3">Crediti fotografici</h1>
        ...
        <ul className="divide-y divide-ink/10 border-y border-ink/10">
          {PHOTO_CREDITS.map((p) => (
            <li key={p.file} className="py-3.5 flex flex-wrap items-baseline gap-x-3 gap-y-1">
              ...
              <a href={p.url} target="_blank" rel="noopener noreferrer" className="font-alt text-xs font-semibold text-alga-600 hover:text-alga underline underline-offset-2">
                Commons →
              </a>
            </li>
          ))}
        ...
        <Link href="/" className="mt-10 inline-flex font-alt text-sm font-semibold text-alga-600 hover:text-alga underline underline-offset-2">
          ← Torna alla home
        </Link>
      </div>
    </div>
