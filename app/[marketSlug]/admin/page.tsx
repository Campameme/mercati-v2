    ...
    <div className="min-h-screen bg-crema">
      <div className="container mx-auto px-4 md:px-6 py-10 md:py-14 max-w-5xl">
        ...
        <section className="mb-12">
          <p className="text-xs font-alt uppercase tracking-wider text-ink-muted mb-4">Configurazione zona</p>
            ...
            {cards.map((c) => {
              ...
              return (
                <Link
                  key={c.href}
                  href={c.href}
                  className="group bg-white border-2 border-ink/10 rounded-xl p-5 hover:border-alga hover:-translate-y-0.5 transition-all"
                >
                  <Icon className="w-8 h-8 text-alga mb-3" />
                  <h2 className="font-alt font-bold text-lg text-ink mb-1 group-hover:text-alga-600 transition-colors">{c.title}</h2>
        ...
        <section>
                    ...
                    <div className="flex items-center justify-between gap-3 px-4 md:px-5 py-4 border-b-2 border-ink/10 flex-wrap">
                      <div className="min-w-0">
                        <h3 className="font-alt font-bold text-xl text-ink flex items-center gap-2 min-w-0">
                          <MapPin className="w-4 h-4 text-alga flex-shrink-0" />
                      ...
                      <Link
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-2 bg-crema border-2 border-ink/15 hover:border-alga text-ink rounded-full text-xs font-alt uppercase tracking-wider transition-colors"
                      >
                          ...
                          return (
                            <li key={s.id}>
                              <Link
                                ...
                                className="group grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-5 items-center py-3 md:py-4 px-4 md:px-5 hover:bg-crema transition-colors"
                              >
                                <div className="md:col-span-4 flex items-center gap-2.5 min-w-0">
                                  ...
                                  />
                                  <span className="font-alt text-base text-ink truncate">{s.giorno}</span>
                                </div>
                                ...
                                <div className="md:col-span-4 flex items-center justify-between md:justify-end gap-3">
                                  ...
                                  <ArrowRight className="w-4 h-4 text-ink-muted group-hover:text-alga-600 group-hover:translate-x-0.5 transition-all" />
                                </div>
                              </Link>
                            </li>
                          )
          ...
          {unboundCount > 0 && (
            <p className="mt-4 text-xs text-ink-muted">
              ...
              <Link href={`/${market.slug}/admin/operators`} className="underline ml-1 hover:text-alga-600">Tutti i banchi</Link>.
            </p>
          )}
        </section>
      </div>
