...
const PRIO_COLOR: Record<NewsPriority, string> = { low: 'bg-crema text-ink-muted border-2 border-ink/15', medium: 'bg-alga/15 text-alga-600 border-2 border-alga/30', high: 'bg-terracotta/15 text-terracotta-600 border-2 border-terracotta/30' }
                ...
                <div key={n.id} className="p-4 flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      {n.is_global && (
                        <span className="text-[11px] font-alt uppercase tracking-wider px-2 py-0.5 rounded-full bg-terracotta/20 text-ink border-2 border-terracotta flex items-center gap-1">
                          <Globe2 className="w-3 h-3" /> globale
                        </span>
                      )}
                      <span className={`text-xs ${live ? 'text-alga-600' : 'text-ink-muted'}`}>{live ? '● pubblicata' : '○ non visibile ora'}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button onClick={() => setEditing(n)} className="p-2 text-ink-muted hover:text-alga-600 transition-colors" title="Modifica"><Pencil className="w-4 h-4" /></button>
          ...
          <div className="fixed inset-0 z-50 bg-ink/60 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl border-2 border-ink/10 shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6 space-y-4">
                {role === 'super_admin' && (
                  <div className="p-3 rounded-xl border-2 border-terracotta bg-terracotta/15">
                ...
                <label className="block">
                  ...
                  <textarea value={editing.content ?? ''} onChange={(e) => setEditing({ ...editing, content: e.target.value })} rows={5} className="w-full mt-1 px-3 py-2 bg-crema border-2 border-ink/15 rounded-xl text-ink focus:outline-none focus:border-alga transition-colors" />
                </label>
