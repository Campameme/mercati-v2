...
export default function OperatorProductsPage() {
  ...
  if (!operatorId) return <div className="min-h-screen bg-carta"><div className="container mx-auto px-4 py-8 text-ink-soft">Nessun operatore collegato al tuo account.</div></div>
            ...
            {products.map((p) => (
              <div key={p.id} className="bg-white rounded-xl border-2 border-ink/10 overflow-hidden flex flex-col">
                {p.photos?.[0] ? (
                  ...
                  <div className="w-full h-40 bg-carta flex items-center justify-center text-ink-muted text-sm">Nessuna foto</div>
                )}
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  {p.price !== null && (
                    <p className="text-mare-700 font-alt font-bold text-lg mt-2">
                      {new Intl.NumberFormat('it-IT', { style: 'currency', currency: p.currency }).format(p.price)}
                    </p>
                  )}
                  <div className="flex items-center gap-2 mt-3">
                    <button onClick={() => setEditing(p)} className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-carta border-2 border-ink/15 hover:border-mare text-ink rounded-full text-sm transition-colors">
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <label className="block md:col-span-2">
                    ...
                    <input value={editing.name} onChange={(e) => setEditing({ ...editing, name: e.target.value })} className="w-full mt-1 px-3 py-2 bg-carta border-2 border-ink/15 rounded-xl text-ink focus:outline-none focus:border-mare transition-colors" />
                  </label>
                ...
                <label className="block">
                  ...
                  <textarea value={editing.description ?? ''} onChange={(e) => setEditing({ ...editing, description: e.target.value })} rows={3} className="w-full mt-1 px-3 py-2 bg-carta border-2 border-ink/15 rounded-xl text-ink focus:outline-none focus:border-mare transition-colors" />
                </label>
                <div>
