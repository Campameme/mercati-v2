...
export default function OperatorProductsPage() {
  ...
  if (!operatorId) return <div className="min-h-screen bg-paper"><div className="container mx-auto px-4 py-8 text-ink-soft">Nessun operatore collegato al tuo account.</div></div>
            ...
            {products.map((p) => (
              <div key={p.id} className="bg-white rounded-xl border-2 border-ink/10 overflow-hidden flex flex-col">
                {p.photos?.[0] ? (
                  ...
                  <div className="w-full h-40 bg-paper flex items-center justify-center text-ink-muted text-sm">Nessuna foto</div>
                )}
          ...
          <div className="fixed inset-0 z-50 bg-night/60 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl border-2 border-ink/10 shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between p-6 border-b-2 border-ink/10">
                <h2 className="font-display text-xl text-ink">Modifica prodotto</h2>
